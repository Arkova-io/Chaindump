...
- Queue only a FIELD-LEVEL EVIDENCE CANDIDATE. The payload MUST include entity_id, field_path, claim, as_of (YYYY-MM-DD), and source_refs (the exact source ids supporting that claim). Each source MUST include id, title, URL, source_type, verified_at (timezone-qualified ISO timestamp), and verification_result="resolved". Include existing value if known, source-published date when available, causal reasoning, counterevidence/unknowns, and suggested reviewer action.
