...
const document = JSON.parse(readFileSync(
  new URL('../docs/exchange-wave2-2026-07-29.json', import.meta.url),
  'utf8',
));
const migration = readFileSync(
  new URL('../migrations/0034_exchange_wave2_forensic_analysis.sql', import.meta.url),
  'utf8',
);
...
function migrationDocument() {
  ...
  if (!match) throw new Error('0034 canonical payload not found');
...
function createFixture() {
  ...
  database.exec(`
    ...
    INSERT INTO successful_exchanges VALUES
      ('aerodrome', 'dex', '{"preserved":"aerodrome"}', 'old'),
      ('pancakeswap', 'dex', '{"preserved":"pancakeswap"}', 'old'),
      ('untouched-success', 'dex', '{"preserved":"untouched"}', 'old');
    INSERT INTO mid_exchanges VALUES
      ('osmosis', 'dex', '{"preserved":"osmosis"}', 'old'),
      ('untouched-mid', 'dex', '{"preserved":"untouched"}', 'old');
    INSERT INTO dead_exchanges VALUES
      ('mango-markets', 'dex', '{"preserved":"mango-markets"}', 'old'),
      ('bittrex', 'cex', '{"preserved":"bittrex"}', 'old'),
...
describe('exchange forensic Wave-2', () => {
  ...
  it('attaches each analysis, preserves profile data, and replays idempotently', () => {
    ...
    expect(database.prepare(`
      ...
      FROM sqlite_master
      WHERE type = 'table' AND name = 'exchange_wave2_forensic_0034'
    `).get().count).toBe(0);
  });
});
