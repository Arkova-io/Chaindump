...
const document = JSON.parse(readFileSync(
  new URL('../docs/nft-wave6-2026-07-29.json', import.meta.url),
  'utf8',
));
const migration = readFileSync(
  new URL('../migrations/0046_nft_wave6_lifecycle.sql', import.meta.url),
  'utf8',
);
const expectedSlugs = [
  'world-of-women',
...
const previousCases = new Set([
  ...
  'ethereum-name-service', 'cryptokitties', 'the-sandbox-land', 'pizza-ninjas',
  'nifty-gateway',
]);
...
describe('NFT and Ordinals Wave-6 lifecycle dossiers', () => {
  it('publishes five non-overlapping lifecycle cases through citation and freshness gates', () => {
    ...
    expect(document.dossiers.map(({ status }) => status)).toEqual([
      'middling', 'thriving', 'thriving', 'middling', 'thriving',
    ]);
  ...
  it('preserves supply, mint, lifecycle and scope boundaries', () => {
    ...
    expect(cases['world-of-women'].profile.supply).toContain('10,000');
    expect(cases['world-of-women'].profile.supply).toContain('22,222');
    expect(cases['cool-cats'].profile.supply).toContain('9,999');
    expect(cases['cool-cats'].profile.analysis).toContain('does not establish');
    expect(cases.cyberkongz.profile.supply).toContain('1,000 Genesis');
    expect(cases.cyberkongz.profile.supply).toContain('20,000 Genkai');
    expect(cases.bitmap.profile.supply).toContain('Expanding rather than fixed');
    expect(cases.bitmap.profile.mint_price).toContain('No protocol-wide sale');
    expect(cases['opensea-marketplace'].profile.supply).toContain('Not applicable');
    expect(cases['opensea-marketplace'].profile.analysis).toContain('does not establish');
    ...
    for (const dossier of document.dossiers) {
      ...
      expect(dossier.profile.market_holder_boundaries.market)
        .toMatch(/^No current|^No independently/);
      expect(dossier.profile.market_holder_boundaries.holders)
        .toMatch(/^No current|^No independently|Not a collection/);
    }
  });
