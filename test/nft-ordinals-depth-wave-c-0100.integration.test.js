...
import {
  ...
  profiles,
  renderMigration,
} from '../scripts/render-nft-ordinals-depth-wave-c-0100.mjs';

const artifact = JSON.parse(readFileSync(
  new URL('../docs/nft-ordinals-depth-wave-c-2026-08-03.json', import.meta.url),
  'utf8',
));
const migration = readFileSync(
  new URL('../migrations/0100_nft_ordinals_depth_wave_c.sql', import.meta.url),
  'utf8',
);
const html = readFileSync(new URL('../public/index.html', import.meta.url), 'utf8');
const TARGETS = Object.freeze([
  ['nft_collection', 'funko-digital-pop'],
  ['nft_collection', 'sorare-cards'],
  ['nft_collection', 'metroverse'],
  ['nft_collection', 'nouns-dao'],
  ['ordinals_collection', 'twelvefold'],
]);
...
describe('NFT and Ordinals normalized-depth Wave C migration 0100', () => {
  ...
  it('keeps causes, scope, lifecycle and unknowns inside their evidence boundaries', () => {
    expect(profiles['funko-digital-pop'].analysis.sections.why_this_outcome.body)
      .toMatch(/strongest hook was also its central weakness|did not publish program revenue/i);
    expect(profiles['funko-digital-pop'].status.operating_state)
      .toBe('operator_program_ceased_2026_assets_may_persist_elsewhere');

    expect(profiles['sorare-cards'].analysis.sections.why_this_outcome.body)
      .toMatch(/repeat use in live competitions|continued competitions.*did not simply collapse/i);
    expect(profiles['sorare-cards'].analysis.sections.risks_and_unknowns.body)
      .toMatch(/prosecution allegation, not a conviction/i);

    expect(profiles.metroverse.analysis.sections.why_this_outcome.body)
      .toMatch(/completed features existed without a retained audience|96 percent traffic decline/i);
    expect(profiles.metroverse.extensions.editorial_guardrail)
      .toMatch(/Do not call Metroverse a rug pull/i);

    expect(profiles['nouns-dao'].analysis.sections.why_this_outcome.body)
      .toMatch(/treasury creates an arbitrage problem|Durable contracts.*not agreement/i);
    expect(profiles['nouns-dao'].analysis.sections.token_and_value_capture.body)
      .toMatch(/not automatically to existing holders|not an everyday redemption guarantee/i);

    expect(profiles.twelvefold.analysis.sections.lifecycle.body)
      .toMatch(/successful primary art release.*unclassified current collector economy/i);
    expect(profiles.twelvefold.extensions.editorial_guardrail)
      .toMatch(/Do not describe the 2023 auction as current liquidity/i);
  });
  ...
  it('records source access and independence honestly', () => {
    for (const [slug, value] of Object.entries(profiles)) {
      ...
      for (const source of value.sources) {
        ...
        expect(source.checked_at, source.id).toBe('2026-08-03T22:03:00Z');
  ...
  it('changes only five profiles, preserves legacy data and replays idempotently', () => {
    ...
    expect(database.prepare(`SELECT COUNT(*) AS count FROM sqlite_master WHERE type = 'table' AND name = '_nft_ordinals_depth_wave_c_0100'`).get().count).toBe(0);
    database.close();
  });
