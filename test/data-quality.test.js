...
describe('the rule refuses verdicts it cannot justify', () => {
  ...
  it('does NOT auto-brand a small chain — the live Gala false positive', () => {
    // Gala: $4.2M, 100% in one first-party DEX, DefiLlama shows no audit and no
    // indexed bridge. It was branded "cannot independently verify this TVL" on its
    // public share card — a false adverse claim on a real project, generated by a
    // DefiLlama coverage gap. A $4.2M chain is not mistaken for a major peer.
    const gala = [{ name: 'Gala Swap', category: 'Dexs', audits: '0', chainTvls: { Gala: 4_200_000 } }];
    const dq = assessChainDataQuality('Gala', gala, { displayedTvl: 4_200_000 });
    expect(dq).toBeTruthy();              // the rule still computes it (an agent may ask)
    expect(dq.autoPublish).toBe(false);   // but it is NOT published to a card/board/profile
    expect(AUTO_PUBLISH_TVL_MIN).toBe(25e6);
  });

  it('the floor sits between the live false-positive set and the one true positive', () => {
    // Measured live 2026-07-17: 55 chains fire; Anubis ($200M) is the only one
    // above Strato at $15.8M. The floor must clear the false-positive set and
    // stay well under Anubis.
    expect(AUTO_PUBLISH_TVL_MIN).toBeGreaterThan(15.8e6);   // above the largest false positive
    expect(AUTO_PUBLISH_TVL_MIN).toBeLessThan(200e6);       // below the one true positive
    const mk = (tvl) => assessChainDataQuality('X', [{ name: 'D', category: 'Dexs', audits: '0', chainTvls: { X: tvl } }], { displayedTvl: tvl });
    expect(mk(15.8e6).autoPublish).toBe(false);   // Strato-sized: held
    expect(mk(200e6).autoPublish).not.toBe(false); // Anubis-sized: published
  });
