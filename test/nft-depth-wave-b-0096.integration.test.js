...
import {
  ...
  profiles,
  renderMigration,
} from '../scripts/render-nft-depth-wave-b-0096.mjs';

const artifact = JSON.parse(readFileSync(
  new URL('../docs/nft-depth-wave-b-2026-08-03.json', import.meta.url),
  'utf8',
));
const migration = readFileSync(
  new URL('../migrations/0096_nft_depth_wave_b.sql', import.meta.url),
  'utf8',
);
const html = readFileSync(new URL('../public/index.html', import.meta.url), 'utf8');
const TARGETS = Object.freeze([
  ['nft_collection', 'cryptopunks'],
  ['nft_collection', 'reddit-collectible-avatars'],
  ['nft_collection', 'moonbirds'],
  ['nft_collection', 'kraken-nft-marketplace'],
  ['ordinals_collection', 'nodemonkes'],
]);
...
describe('NFT and Ordinals depth wave B migration 0096', () => {
  it('keeps the research artifact and D1-safe migration deterministic', () => {
  ...
  it('ships one normalized ten-section contract with atomic dated claims', () => {
    for (const [type, slug] of TARGETS) {
      ...
      expect(value.claims.length, slug).toBeGreaterThanOrEqual(35);
    }
  });

  it('keeps the lifecycle calls inside the evidence boundary', () => {
    expect(profiles.cryptopunks.analysis.sections.why_this_outcome.body)
      .toMatch(/historical object stable|do not prove.*prices or liquidity/i);
    expect(profiles.cryptopunks.analysis.sections.token_and_value_capture.body)
      .toMatch(/no collection-specific fungible token|no disclosed project take rate/i);

    expect(profiles['reddit-collectible-avatars'].analysis.sections.lifecycle.body)
      .toMatch(/product sunset|not total erasure/i);
    expect(profiles['reddit-collectible-avatars'].metrics[0].quality_flags)
      .toContain('includes_free_distributions');

    expect(profiles.moonbirds.analysis.sections.why_this_outcome.body)
      .toMatch(/third owner|wider Vibes and birb ecosystem/i);
    expect(profiles.moonbirds.metrics.every(({ quality_flags }) => (
      quality_flags.includes('not_moonbirds_nft_specific')
    ))).toBe(true);

    expect(profiles['kraken-nft-marketplace'].analysis.sections.lifecycle.body)
      .toMatch(/failed product with an announced wind-down|does not imply.*Kraken itself ceased/i);
    expect(profiles['kraken-nft-marketplace'].metrics).toEqual([]);

    expect(profiles.nodemonkes.analysis.sections.why_this_outcome.body)
      .toMatch(/no-roadmap message|not durable liquidity/i);
    expect(profiles.nodemonkes.analysis.sections.token_and_value_capture.body)
      .toMatch(/do not establish an official fungible token|not every owner/i);
    expect(profiles.nodemonkes.metrics[0]).toMatchObject({
      dimension: 'mint_raise', value: 240, currency: 'BTC',
    });
  });

  it('records source access and causal-source independence honestly', () => {
    for (const [slug, value] of Object.entries(profiles)) {
      ...
      for (const source of value.sources) {
        ...
        expect(source.checked_at, source.id).toBe('2026-08-03T21:25:00Z');
        expect(['reachable', 'archived'], source.id).toContain(source.access_state);
        expect(source.independence_group, source.id).not.toBe('');
        if (source.direct_http_status === 403 || source.direct_http_status === null) {
  ...
  it('changes only five profiles, preserves legacy data and replays idempotently', () => {
    ...
    expect(database.prepare(`SELECT COUNT(*) AS count FROM sqlite_master WHERE type = 'table' AND name = '_nft_depth_wave_b_0096'`).get().count).toBe(0);
    database.close();
  });
