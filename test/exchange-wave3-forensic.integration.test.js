...
const document = JSON.parse(readFileSync(
  new URL('../docs/exchange-wave3-2026-07-29.json', import.meta.url),
  'utf8',
));
const migration = readFileSync(
  new URL('../migrations/0035_exchange_wave3_forensic_analysis.sql', import.meta.url),
  'utf8',
);
...
function migrationDocument() {
  ...
  if (!match) throw new Error('0035 canonical payload not found');
...
function createFixture() {
  ...
  database.exec(`
    ...
    INSERT INTO successful_exchanges VALUES
      ('thorchain', 'dex', '{"preserved":"thorchain"}', 'old'),
      ('jupiter', 'dex', '{"preserved":"jupiter"}', 'old'),
      ('untouched-success', 'dex', '{"preserved":"untouched"}', 'old');
    INSERT INTO mid_exchanges VALUES
      ('balancer', 'dex', '{"preserved":"balancer"}', 'old'),
      ('kucoin', 'cex', '{"preserved":"kucoin"}', 'old'),
      ('untouched-mid', 'dex', '{"preserved":"untouched"}', 'old');
    INSERT INTO dead_exchanges VALUES
      ('platypus-finance', 'dex', '{"preserved":"platypus-finance"}', 'old'),
...
describe('exchange forensic Wave-3', () => {
  ...
  it('attaches each analysis, preserves profile data, and replays idempotently', () => {
    ...
    expect(database.prepare(`
      ...
      FROM sqlite_master
      WHERE type = 'table' AND name = 'exchange_wave3_forensic_0035'
    `).get().count).toBe(0);
  });
});
