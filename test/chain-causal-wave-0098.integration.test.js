...
import { document, renderMigration } from '../scripts/render-chain-causal-wave-0098.mjs';
import {
...
const artifact = JSON.parse(readFileSync(
  new URL('../docs/chain-causal-completion-wave-j-2026-08-03.json', import.meta.url),
  'utf8',
));
const migration = readFileSync(
  new URL('../migrations/0098_chain_causal_completion_wave_j.sql', import.meta.url),
  'utf8',
);
const expected = {
  polygon: { chain: 'Polygon', outcome: 'middling', forensic: 'middling', sources: 12, claims: 33, tvl: 800_281_101 },
  'robinhood-chain': { chain: 'Robinhood Chain', outcome: 'middling', forensic: 'middling', sources: 13, claims: 29, tvl: 401_009_081 },
  arbitrum: { chain: 'Arbitrum', outcome: 'middling', forensic: 'middling', sources: 12, claims: 33, tvl: 1_186_527_959 },
  tron: { chain: 'Tron', outcome: 'thriving', forensic: 'thriving', sources: 13, claims: 33, tvl: 4_885_588_675 },
  avalanche: { chain: 'Avalanche', outcome: 'middling', forensic: 'middling', sources: 13, claims: 33, tvl: 435_809_014 },
};
...
describe('chain causal and canonical wave 0098', () => {
  ...
  it('keeps facts, inferences, unknowns and point-in-time observations explicit', () => {
    for (const entry of document.cases) {
      ...
      expect(entry.forensic_analysis.observation_snapshot).toMatchObject({
        observed_at: '2026-08-03T21:32:28.000Z',
        observation_completed_at: '2026-08-03T21:32:52.000Z',
    ...
    const polygon = document.cases.find(({ slug }) => slug === 'polygon').canonical_profile;
    expect(polygon.analysis.sections.what_it_is.body).toContain('does not make Polygon PoS an Ethereum rollup');
    expect(polygon.metrics.find(({ dimension }) => dimension === 'stablecoin_supply').quality_flags)
      .toContain('not_equivalent_to_tvl');
    const robinhood = document.cases.find(({ slug }) => slug === 'robinhood-chain').canonical_profile;
    expect(robinhood.analysis.sections.token_and_value_capture.body).toContain('no documented native token');
    expect(robinhood.metrics.find(({ dimension }) => dimension === 'dex_spot_volume').quality_flags)
      .toContain('launch_base_effect');
    const arbitrum = document.cases.find(({ slug }) => slug === 'arbitrum').canonical_profile;
    expect(arbitrum.analysis.sections.what_it_is.body).toContain('Orbit chains');
    expect(arbitrum.metrics.find(({ dimension }) => dimension === 'dex_spot_volume').quality_flags)
      .toContain('orbit_chains_excluded');
    const tron = document.cases.find(({ slug }) => slug === 'tron').canonical_profile;
    expect(tron.analysis.sections.what_happened.body).toContain('Stablecoin transfer use, not DEX volume');
    expect(tron.metrics.find(({ dimension }) => dimension === 'stablecoin_supply').quality_flags)
      .toContain('not_transfer_volume');
    const avalanche = document.cases.find(({ slug }) => slug === 'avalanche').canonical_profile;
    expect(avalanche.analysis.sections.what_it_is.body).toContain('not rollups');
    expect(avalanche.metrics.find(({ dimension }) => dimension === 'dex_spot_volume').quality_flags)
      .toContain('l1_activity_excluded_or_separate');
  });

  it('preserves every legacy field, merges sources by URL and replays idempotently', () => {
    const database = new DatabaseSync(':memory:');
    applyMigrations(database, 97);
