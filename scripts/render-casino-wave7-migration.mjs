...
const sourceUrl = new URL('../docs/casino-wave7-2026-07-29.json', import.meta.url);
const outputUrl = new URL('../migrations/0045_casino_wave7.sql', import.meta.url);
const document = JSON.parse(readFileSync(sourceUrl, 'utf8'));
const payload = JSON.stringify(document).replaceAll("'", "''");

const sql = `-- Generated from docs/casino-wave7-2026-07-29.json.
-- The fixed-key writes are idempotent. The staging table is removed at the end.

CREATE TABLE IF NOT EXISTS casino_wave7_payload_0045 (
  document TEXT NOT NULL CHECK (json_valid(document))
);

DELETE FROM casino_wave7_payload_0045;

-- canonical-payload-start
INSERT INTO casino_wave7_payload_0045 (document)
VALUES ('${payload}');
...
FROM casino_wave7_payload_0045 AS payload,
...
FROM casino_wave7_payload_0045;

DROP TABLE IF EXISTS casino_wave7_payload_0045;
`;
