...
const augurSources = [
  ...
  source(augur, 'migration', 'Phase 2: The Fork Migration', 'https://www.augur.net/blog/phase-2-the-fork-migration', 'Lituus Foundation / Augur', { publishedAt: '2026-06-05', locator: 'Official phase-two notice tells holders to migrate by or until August 3, which conflicts with the August 1 language in other official material.' }),
  source(augur, 'migration-guide', 'Moon Fork Migration Guide', 'https://www.augur.net/learn/fork/migration/', 'Lituus Foundation / Augur', { locator: 'Official guide says migrate before August 1 while its schedule says late migration may finish between August 2 and August 10; this internal timing conflict prevents a settled window-status call.' }),
  source(augur, 'faq', 'Fork and migration FAQ', 'https://www.augur.net/faq/', 'Lituus Foundation / Augur', { locator: 'FAQ documents the fork, per-universe tokens, one-way migration and an August 1 cutoff, conflicting with the phase-two notice and migration-guide schedule.' }),
...
const specs = [
  ...
  },
  {
    ...
    unknowns: ['authoritative migration cutoff and whether the window remained open', 'REP migration completion and supply by universe', 'current production markets and users', 'open interest and settled volume', 'liquidity and fees', 'production oracle reliability', 'frontend and market-creator adoption', 'jurisdiction-specific operators and legal access', 'custody outside named exchanges'],
    unsourcedFields: ['authoritative_migration_cutoff', 'migration_window_state', 'migration_completion', 'supply_by_universe', 'active_markets', 'active_users', 'open_interest', 'settled_volume', 'liquidity', 'fees', 'oracle_reliability', 'frontend_adoption', 'legal_access', 'custody'],
    methodologyNotes: [
      'Repository activity proves current engineering, not production readiness or commercial adoption.',
      'Official pages conflict on the cutoff: August 1 in the FAQ and July update, August 3 in the phase-two notice, and an August 2–10 completion range in the migration guide. Window status is not inferred on the August 3 review date.',
    ...
    sections: {
      ...
      what_happened: section(`Augur launched its original mainnet in 2018 and later lost normal product momentum. In April 2026, the Lituus Foundation intentionally triggered a fork to test the mechanism and coordinate a reboot. Phase two opened a one-way token-migration window, but official pages published conflicting cutoffs: August 1, August 3 and a late-migration completion range extending to August 10. On this August 3 review, ForkWatch showed a zero countdown while migration progress remained unavailable. Kraken separately expected to convert supported customer balances by July 31. Development is live; the authoritative cutoff, window state and migration result are unresolved.`, [
        ...
        claim('Official pages conflict on the migration cutoff, and neither window state nor total completion was verifiable on 2026-08-03.', [sid(augur, 'faq'), sid(augur, 'migration'), sid(augur, 'migration-guide'), sid(augur, 'forkwatch')], 'Conflicting official timing language and unavailable dashboard progress.', { kind: 'unknown' }),
      ]),
      ...
      operating_model: section(`Historically, users created or traded prediction markets while designated participants reported outcomes and REP holders could dispute them. The 2026 fork creates multiple universes, and REP can move only one way into a chosen universe during the migration window. Official pages do not agree on the window’s cutoff. Lituus is the new implementation workstream; the Moon Fork is the test and coordination event; a frontend would still need to provide markets, trading and support. Self-custodied holders handle their own migration, while Kraken may migrate balances it custodies under its own process. Current production market and settlement service levels are not established.`, [
      ...
      token_and_value_capture: section(`REP historically supported reporting and dispute participation rather than representing equity in a casino operator. During the 2026 fork, old REP and REPV2 holders had to move to a universe-specific token, and the selected canonical asset is described as AUGUR. Kraken expected a 1:1 conversion for eligible balances on its platform, but that does not measure self-custodied or other-exchange migration. Conflicting official cutoffs create stranded-token risk without proving the window was closed on August 3. No current source establishes total migrated supply, token concentration, fees, treasury income, holder distributions or a claim on future products.`, [
      ...
      risks_and_unknowns: section(`The immediate risk is migration uncertainty. The FAQ and July update use August 1, the phase-two notice uses August 3, and the migration guide says late migration may finish between August 2 and August 10. ForkWatch did not expose a usable completion number on August 3, so neither closure nor continuation is asserted. Some holders relied on custodians, while self-custodied holders bore execution risk. The rebuild also faces contract, oracle, governance, frontend and liquidity risk. Current markets, traders, open interest, fees, migrated supply, reliability and legal access are unknown.`, [
        claim('Official timing conflicts prevent a verified migration-window or completion call on 2026-08-03.', [sid(augur, 'faq'), sid(augur, 'current'), sid(augur, 'migration'), sid(augur, 'migration-guide'), sid(augur, 'forkwatch')], 'Conflicting official cutoffs and unavailable progress.', { kind: 'unknown' }),
      ...
      lifecycle: section(`Augur published its original protocol paper in January 2015 and launched mainnet in 2018. After the original market lost momentum, the Lituus Foundation triggered the Moon Fork on April 8, 2026 and opened phase-two migration in June. Its materials then published August 1, August 3 and August 2–10 timing. On July 21 it reported the fork live and ChainSafe implementation work underway. By August 3, repositories had fresh activity, but ForkWatch did not reveal a completed migration share or commercial market. The lifecycle is active technical recovery with window state, adoption and migration outcome unproven.`, [
    ...
    events: [
      ...
      event('migration-timing-conflict', 'timeline_conflict_observation', AS_OF, 'Official Augur materials published August 1, August 3 and August 2–10 migration timing; window state and completion were unresolved.', [sid(augur, 'faq'), sid(augur, 'migration'), sid(augur, 'migration-guide'), sid(augur, 'forkwatch')], 'Conflicting official timing language and unavailable progress on the review date.'),
