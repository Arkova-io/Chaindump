...
const sourceUrl = new URL('../docs/casino-wave5-2026-07-29.json', import.meta.url);
const outputUrl = new URL('../migrations/0039_casino_wave5.sql', import.meta.url);
const document = JSON.parse(readFileSync(sourceUrl, 'utf8'));
const payload = JSON.stringify(document).replaceAll("'", "''");

const sql = `-- Generated from docs/casino-wave5-2026-07-29.json.
-- The fixed-key writes are idempotent. The staging table is removed at the end.

CREATE TABLE IF NOT EXISTS casino_wave5_payload_0039 (
  document TEXT NOT NULL CHECK (json_valid(document))
);

DELETE FROM casino_wave5_payload_0039;

-- canonical-payload-start
INSERT INTO casino_wave5_payload_0039 (document)
VALUES ('${payload}');
...
FROM casino_wave5_payload_0039 AS payload,
...
FROM casino_wave5_payload_0039;

DROP TABLE IF EXISTS casino_wave5_payload_0039;
`;
