...
import { buildWaveHSpecs } from './chain-causal-wave-h-specs.mjs';

const root = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const artifactPath = resolve(root, 'docs/chain-causal-completion-wave-h-2026-08-03.json');
const migrationPath = resolve(root, 'migrations/0091_chain_causal_completion_wave_h.sql');
...
const specs = buildWaveHSpecs({ source, claim, section, metric, event, marketSources, AS_OF, OBSERVED_AT, ACCESSED_AT });
...
const document = {
  ...
  methodology: {
    scope: 'Canonical ten-section blockchain profiles for Celo, Doma, Fraxtal, Gala and Internet Computer.',
    ...
    preservation_rule: 'Migration 0091 preserves legacy facts and analysis fields while adding forensic analysis, review metadata and an embedded canonical profile. Sources merge by URL.',
  },
  cases,
};
...
function renderMigration(value = document) {
  ...
  return `-- Generated by scripts/render-chain-causal-wave-0091.mjs.
-- Adds Wave H review-state canonical profiles without overwriting legacy dossiers.
