...
VALUES (
  ${quoteSql(entry.table)},
  ${quoteSql(entry.kind)},
  ${quoteSql(entry.lifecycle)},
  ${quoteSql(entry.slug)},
  ${quoteSql(JSON.stringify(entry.forensic_analysis))},
  ${quoteSql(JSON.stringify(entry.sources))},
  ${entry.replace_profile ? quoteSql(JSON.stringify(entry.replace_profile)) : 'NULL'},
  ${entry.row_patch ? quoteSql(JSON.stringify(entry.row_patch)) : 'NULL'}
);`).join('\n');
