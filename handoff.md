...
- Wave 2 explicit evidence boundary: live CoinGecko/DefiLlama fields were observed on 2026-07-29; source URLs and dates are stored per dimension. Unknown capital, treasury, founder, developer, active-address, concentration, and stablecoin fields remain `null` and appear in each dossier’s `unsourced_fields` rather than being inferred.
 - Merged PR #52 adds cited successful-CEX controls for Coinbase, Kraken, Binance, Bybit, and Bitstamp. It keeps issuer metrics in five distinct comparison classes, records BNB separately from the other venue-token unknowns, and does not pool volume/revenue definitions. Bybit and Bitstamp intentionally show `review_due`, not `current`, because their lifecycle evidence is from 2025.
